...
| `c22-nulle-horizon.patch` | **CANDIDAT EN MESURE** : le test de nulle passe avant l'aiguillage vers la quiescence, et la règle des cinquante coups cède devant le mat. Avec ses trois tests, qui échouent sur l'ancien code. Même contenu que le commit `05a9dc4`, révoqué aussitôt pour ne pas retenir la branche | **en mesure** — deux jobs de 3000 parties à `8+0,08`, 23 sept. 2026. Critère écrit avant de lancer : fusion sauf régression significative (voir `tools/README.md`, section C22) | `git apply --check` : **oui**, sur `main` |
| `c23-sonde-fenetre-coup-nul.patch` | sonde : combien des répétitions que voit la recherche ne tiennent qu'en remontant AU-DELÀ d'un coup nul ? Contient son lecteur, `tools/sonde-c23/` — `rejouer.py`, copie de celui de C22, et `sommer.py` | **pas un changement** — **87,8 %** des répétitions intérieures et **92,1 %** de celles de l'horizon sont fausses, à 87 à 95 % par deux coups nuls consécutifs. 30 parties à `8+0,08`, régime réel, 23 sept. 2026 | `git apply --check` : **oui**, sur `main` ; **non** une fois C22 fusionné (même contexte qu'`is_repetition`) |
| `c23-fenetre-coup-nul.patch` | **CANDIDAT EN MESURE** : la fenêtre de répétition s'arrête au dernier coup nul, comme `pliesFromNull` chez Stockfish. Avec ses quatre tests, et quatre défauts injectés tous attrapés. Même contenu que le commit `3236f12`, révoqué aussitôt | **en mesure** — critère écrit avant de lancer : fusion sauf régression significative (voir `tools/README.md`, section C23) | `git apply --check` : **oui**, sur `main` ; après C22, par `git apply -C1` seulement — vérifié, les tests des deux passent |
