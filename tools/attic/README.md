...
| `c22-nulle-horizon.patch` | **REJETÉ PAR SON CRITÈRE, à remesurer sur C23** : le test de nulle passe avant l'aiguillage vers la quiescence, et la règle des cinquante coups cède devant le mat. Avec ses trois tests, qui échouent sur l'ancien code. Même contenu que le commit `05a9dc4`, révoqué aussitôt pour ne pas retenir la branche | **−10,44 ± 6,34 Elo**, 5 760 parties à `8+0,08`, 23 sept. 2026 — **régression significative, non fusionné**. 92 % des nulles qu'il ajoute à l'horizon sont fausses (C23) : à remesurer sur C23 (voir `tools/README.md`, section C22) | `git apply --check` : **non** depuis la fusion de C23, qui a réécrit le corps d'`is_repetition` juste avant l'endroit où il s'insère ; `git apply --3way` le porte sans conflit |
| `c22-nulle-horizon-sur-c23.patch` | **CANDIDAT EN MESURE** : le code de `c22-nulle-horizon.patch` à l'identique, porté sur `main` d'après C23 — le test de nulle avant l'aiguillage vers la quiescence appelle désormais la fenêtre bornée au dernier coup nul. Même contenu que le commit `cd45ffa`, révoqué aussitôt | **en mesure** — même critère de non-régression que C22 et C23, écrit avant de lancer (voir `tools/README.md`, section « C22 sur C23 ») | `git apply --check` : **oui**, sur `main` |
