...
| **balayage de mutation après B6** | **35949682016** | `main` à `65d0b03`, Lazy SMP | un job par fichier, puis `Verdict` | ~03 h 50 (lancé à 03 h 01) | Prédiction écrite avant, confiance moyenne : `search.rs` ≤ 43, `uci.rs` 0 — chaque opérateur neuf a son test, éprouvé par un défaut injecté ; le garde d'arrêt vidé devrait finir **expiré** et non survivant, un des tests à plusieurs fils pendant alors au lieu d'échouer |
