...
| **balayage de mutation après la PR #82** — C26 fusionné | *à lancer* | `main` à `32a0954` | un job par fichier, puis `Verdict` | ~1 h 30 après le lancement — `search.rs` a pris 90 min au précédent | **Prédiction, écrite avant** : `search.rs` **39**, au plafond, les mêmes survivants un pour un — le crible au candidat n'a trouvé aucun survivant dans `deadlines_ms`, et C26 ne déplace aucun arbre à profondeur fixe ; le compte d'expirés peut bouger avec la charge du runner — les cinq mutants neufs n'en produisent aucun au crible. Tous les autres fichiers à leur plafond, total **139** |
