...
| **C25 — la répartition par la stabilité, la sonde** | **36030127970** | `41d590f` → `910a6c9`, `8+0,08`, 100 parties, une à la fois | cutechess, sonde | ~17 h 50 | La règle écrite avant (section C25) : temps par coup hors de × 0,90 à 1,10, ou une perte au temps → comprendre avant l'Elo ; sinon deux jobs de 3 000 parties. Les plis moyens ne sont plus un critère |
| **balayage de mutation après les PR #78 et #79** — tests corrigés, C24 fusionné | **36028076680** | `main` à `f2dd0cf` | un job par fichier, puis `Verdict` | ~17 h 50, lancé à 16 h 32 | **Prédiction écrite avant** : `search.rs` **39** — les quatre mutants cachés par l'arbre d'A18 attrapés (re-mesuré localement), la garde de `stage_moves` seule en plus des 38 ; C24 n'ajoute rien (crible : aucun survivant). `eval.rs` **≤ 122** : le banc à la profondeur 6 cherche l'itération 5 en entier, donc attrape au moins ce que la profondeur 5 attrapait ; le chiffre exact fixe le plafond — baissé s'il passe sous 121, relevé avec sa raison sinon. Les autres fichiers à leur plafond. Si le verdict est vert : fermer l'issue #69 |
...
### C25 — la répartition par la stabilité : ÉCRIT le 24 sept. 2026, la mesure écrite AVANT de lancer
