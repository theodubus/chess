...
| **balayage de mutation après les PR #78 et #79** — tests corrigés, C24 fusionné | *à lancer* | `main` à `f2dd0cf` | un job par fichier, puis `Verdict` | ~80 min après le lancement | **Prédiction écrite avant** : `search.rs` **39** — les quatre mutants cachés par l'arbre d'A18 attrapés (re-mesuré localement), la garde de `stage_moves` seule en plus des 38 ; C24 n'ajoute rien (crible : aucun survivant). `eval.rs` **≤ 122** : le banc à la profondeur 6 cherche l'itération 5 en entier, donc attrape au moins ce que la profondeur 5 attrapait ; le chiffre exact fixe le plafond — baissé s'il passe sous 121, relevé avec sa raison sinon. Les autres fichiers à leur plafond. Si le verdict est vert : fermer l'issue #69 |
