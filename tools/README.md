...
| **balayage de mutation après la PR #76** | **35976435127**, lancé à 08 h 38 | `main` à `ea8116f` : chaque fil tient le budget de nœuds, `MAX_THREADS` 1 024 ; A18 révoqué, donc absent | un job par fichier, puis `Verdict` | ~1 h | **Prédiction écrite avant** : `search.rs` **38**, inchangé — les dix mutants du code touché, passés en local, sont tous attrapés ou expirés (un budget devenu inatteignable). **Réserve** : `le_budget_de_noeuds_compte_tous_les_fils` n'est plus instable ; s'il attrapait par hasard un mutant équivalent, celui-ci redevient survivant — une hausse de cette nature serait un artefact LEVÉ, à lire mutant par mutant, pas un trou neuf. Les autres fichiers au plafond |
| **C24 — laisser finir l'itération, la sonde** | **35982959972** | `7274844` → `7fc5959`, `8+0,08`, 100 parties, une à la fois | cutechess, sonde | ~10 h 25 | La règle écrite avant (section C24) : temps par coup hors de × 0,90 à 1,10 ou plis entièrement ≤ 0 → pas de match ; sinon deux jobs de 3 000 parties |
...
| **raffinements d'ordonnancement sur les étages** : coup de réfutation, historique de continuation | non chiffrés | **après A18**, dont les étages sont la forme qui les accueille ; aucun n'est décidé. <s>Reléguer les captures perdantes derrière les tranquilles</s> : **+31,6 % de nœuds ici** (C19), ne se rouvre pas sans fait neuf |
