...
| « ce verdict vaut pour le moteur qu'on livrera » | **Nommer la cadence, et savoir ce qu'elle achète.** Mesuré le 16 sept. 2026 : à `1+0,01` le moteur atteint la **profondeur médiane 8,5** ; à ~30+0,3, **17,0**. Les douze premiers verdicts du projet sont tous à `1+0,01` — alors que `tools/sprt.sh` a pour défaut `8+0.08` depuis sa création, défaut jamais utilisé. **Ce n'était pas un arbitrage, c'était une habitude.** Une technique dont la valeur croît avec la profondeur y est sous-évaluée. Mesurer à cadence longue passe par `.github/workflows/match.yml`. |
