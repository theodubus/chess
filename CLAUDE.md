...
- **Les entrées du réseau NNUE sont celles de l'entraîneur, au commit qu'on
  épingle.** Une case retournée d'un seul côté ne fait rien planter : elle
  entraîne un réseau sur d'autres positions que celles qu'il évaluera, et
  aucun test du moteur seul ne le voit. `nnue::feature` suit `Chess768` et
  `ChessBoard::from_raw` de bullet `10e7e82` ;
  `les_entrees_du_moteur_sont_celles_de_l_entraineur`
  (`tools/src/bin/nnue_datagen.rs`) le confronte au CODE de `bulletformat`,
  pas à une lecture de ce code. Changer de version de bullet ou d'entrées
  demande de relire le source et de refaire passer ce test.
- **Un réseau qui pourrait déborder est refusé au chargement, et une
  évaluation n'annonce jamais un mat.** `Network::from_bytes` borne
  l'accumulateur sur 32 pièces et le produit scalaire sur 32 bits ; la sortie
  est bornée à `MATE_THRESHOLD - 1`, et une position morte vaut zéro, réseau
  ou non. Un réseau refusé par `EvalFile` laisse l'évaluation telle qu'elle
  était, et le dit.
...
- **Pile de clés Zobrist** pour la détection de répétition — déjà en place dans
  `Position`.
- **Pile d'accumulateurs NNUE par ply** — **en place depuis le 26 sept. 2026**
  (A21, étape 2) : `Search::accumulators`, la racine recalculée, tout le reste
  DÉRIVÉ par le parent juste avant de descendre — coups, captures de
  quiescence, coup nul. La dérivation vit dans `engine/src/nnue.rs`, seule
  copie ; `nnue_probe.rs` l'emploie. L'accumulateur
...
| « ce réseau est celui qu'a entraîné bullet » | Le réseau quantifié, chargé par le moteur, contre `trainer.eval(fen) × 400` de bullet, sur des positions variées — `tools/nnue-train` le fait lui-même au bout de l'entraînement. **Critère écrit avant le premier entraînement : écart médian ≤ 15 centièmes, maximal ≤ 50**, la quantification en valant une dizaine. Un écart de l'ordre de l'évaluation elle-même dit des entrées mal indexées — et rien d'autre ne le signalerait : un réseau mal branché ne fait rien planter, il joue mal. |
...
- **Une assertion qui ne s'est jamais déclenchée ne prouve rien si aucun
  ...
  <br>**Et un COMPTEUR de passages ne dit pas quels chemins ont été
  parcourus** — 26 sept. 2026, A21. Le test des accumulateurs NNUE comptait
  plus de mille vérifications, et le coup nul oublié passait : le réseau de
  test évaluait tout à ±8 000, la futilité inverse coupait avant le coup nul
...
| `.github/workflows/nnue-train.yml` | à chaque push qui touche `engine/src/nnue.rs`, la crate `tools/nnue-train` ou la toolchain : compile le programme d'entraînement NNUE, `--locked`, sans GPU — contre le runtime factice de bullet. Ce programme a son propre espace de travail et ne tourne que sur la machine d'entraînement : sans ce contrôle, un changement de l'API du moteur le casserait en silence, et on ne l'apprendrait que le jour de l'entraînement |
